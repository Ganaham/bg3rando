...
// Essentially, when writing the part that picks a random subclass and then assigns a corresponding deity, have part that checks for Shart/Lolthdrow
...
const weaponPreference = {
  ...
  // - inspired by fighting style availability.  
  // Fighter has options for all of these, which actually means to me that I don't need to change its weighting at all. 
  // Similarly, characters with no fighting style don't care about any of these. 
  options: [
    {
      name: "light",
      getWeight: (c) => {
    ...
    {
      name: "dontcare", // either no fighting styles or going with non-melee weapon one like Defense or Archery
      getWeight: (c) => {
        return 1;
      }
    }
...
function getDeity(character) {
  if (character.class.name !== "Cleric") return "";
...
function getLoadout(character) {
  const availableWeapons = weaponDatabase.filter(w => w.isValid(character));

  const meleeOptions = availableWeapons.filter(w => w.type !== "ranged");
  const rangedOptions = availableWeapons.filter(w => w.type === "ranged");

  const charWeaponPref = weightFightingStyle(character);

  if (character.class.name === "Rogue") { // Make sure rogues can use sneak attack
    meleeOptions = meleeOptions.filter(w => w.finesse);
  }
  if (character.class.name === "Monk") { // Make sure monks get monk weapon
    meleeOptions = meleeOptions.filter(w => w.type != "twohand");
  }

  // Place where fighting styles can come in. Essentially, paladins are more likely to get a big weapon, rangers more likely to dual wield, etc.
  // Every character also has a chance of landing on "dontcare" which means fighting style is ignored. 
  // Similarly, landing on a weapon from one of these choices doesn't forbid you from using more generalist styles like defense or archery. 
  if (charWeaponPref === "light") {
    meleeOptions = meleeOptions.filter(w => w.type === "light");
  }
  else if (charWeaponPref === "onehand") {
    meleeOptions = meleeOptions.filter(w => w.type === "onehand");
  }
  else if (charWeaponPref === "twohand") {
    meleeOptions = meleeOptions.filter(w => w.type === "twohand");
  }

  // Filter down to only using martial weapons if martial weapons are available
  ...
  if (selectedMelee) {
    if (selectedMelee.type === "light") {
      ...
      meleeText = canUseShield(character) ? `A ${selectedMelee.name} and, optionally, a Shield` : `A ${selectedMelee.name}`;
    } else {
  ...
  return {
    melee: meleeText,
    ranged: selectedRanged ? selectedRanged.name : "None (except Cantrips)"
  };
}

function weightFightingStyle(character) {
  return getFromWeightedList(character, weaponPreference.options);
}

function canUseShield(character) {
  return (["Barbarian", "Cleric", "Druid", "Fighter", "Paladin", "Ranger"].includes(character.class.name)
    || ["Hexblade", "College of Valour"].includes(character.subclass.name));
}
